...
popoutBtn.addEventListener("click", () => ext.runtime.sendMessage({ type: "open-window" }));
...
async function restore() {
  const saved = await ext.storage.local.get(["host", "pairingId", "sessionToken", "connectMode"]);
  ...
  try {
    ...
    if (!found.length) {
      ...
      if (via === "wifi") {
        ...
        setStatus(`USB پیدا نشد.${ips}${extra} IP گوشی را اینجا بنویسید.`, true);
      }
      return;
    }
    ...
    closeSocket();
    await ext.storage.local.set({ host, connectMode: via });
    scanProgress.textContent = "";
...
function connectAgent(host, via) {
  const port = ext.runtime.connect({ name: "phone" });
  agentPort = port;
  agentOpen = false;
  let sent = false;
  const sendConnect = () => {
